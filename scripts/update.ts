...
async function generateFunctionsPage() {
  ...
  for (const fn of functions) {
    const page = basename(fn.file, '.ts')
  ...
  const sections = [...groups.entries()]
    ...
    .map(([category, fns]) => {
      ...
      const rows = [...fns]
        ...
        .map(fn => `- [\`${fn.name}\`](/${fn.pkg}/${basename(fn.file, '.ts')}/) — \`@reaxuse/${fn.pkg}\``)

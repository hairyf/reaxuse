...
- 发布走 npm **Trusted Publishing（OIDC，无长期 token）**：首次发布前需在 npmjs.com 为每个 `@reause/*` 包配置 Trusted Publisher（repository `hairyf/reause`，workflow `publish.yml`），否则 `publish.yml` 会认证失败。

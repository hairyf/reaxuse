...
> **⚠️ 合并冲突矩阵（Round 7 实测，9 个 PR 两两 `git merge-tree`）**：单个 PR 对 `origin/main` 都是 CLEAN/MERGEABLE，但**彼此之间存在 5 组冲突**，全部集中在两个 `index.ts` 的**相邻占位符行**：
> ① `#494 + #496` → `packages/shared/src/index.ts`（占位符行相邻）；
> ② `#497 + #499`、`#497 + #501`、`#498 + #501`、`#499 + #500` → 全部 `packages/integrations/src/index.ts`。
> 规律（已用 36 对组合验证）：**占位符行相距 1 行 → 冲突；相距 ≥2 行 → 自动合并**。integrations 的占用行：#498 useFuse=15、#501 useIDBKeyval=16、#497 useJwt=17、#499 useNProgress=18、#500 useQRCode=19（#502 useCookies=12，与任何 PR 都不冲突）；shared 的 #494/#496 相邻。

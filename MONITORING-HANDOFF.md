...
> ② `#497 + #499`、`#497 + #501`、`#498 + #501`、`#499 + #500`、**`#503 + #504`（占位符行 10/9）** → 全部 `packages/integrations/src/index.ts`。
> 规律（已用 36 对组合 + Round 11 的 28 对复算验证）：**占位符行相距 1 行 → 冲突；相距 ≥2 行 → 自动合并**。integrations 的占用行（Round 11 实测）：#504 useAsyncValidator=9、#503 useAxios=10、#502 useCookies=12、#114 useDrauu=13、#498 useFuse=15、#501 useIDBKeyval=16、#497 useJwt=17、#499 useNProgress=18、#500 useQRCode=19、#209 useSortable=20；shared 的 #494=13/#496=16（相邻）。**Round 11 复算实测（8 个 integrations 分支两两）**：冲突 5 组 = `useasyncvalidator+useaxios`、`usefuse+useidbkeyval`、`useidbkeyval+usejwt`、`usejwt+usenprogress`、`usenprogress+useqrcode`；其余 23 对 OK（含 `useaxios+usecookies` 行距 2 自动合并）。#114 useDrauu(13) 与 #502 useCookies(12) 行距 1 ⇒ 合并后必然冲突；#209 useSortable(20) 与 #500 useQRCode(19) 同理。

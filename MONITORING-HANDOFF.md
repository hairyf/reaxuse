...
* **DOM 元素目标类型（DOM hook 专用，2026-09-08 Round 7 复核）**：`ElementTarget<T extends TargetElement = TargetElement>` 唯一实现于 `packages/core/src/useResizeObserver.ts:17`（另有 `ElementTargetOrArray` 第 24 行），配 `unrefElement`（`packages/core/src/unrefElement.ts`，`ElementTarget<T>` → `TargetElement`）解析 `{ current }` / 元素 / `null`；integrations 侧的 DOM hook 从 `@reaxuse/core` 引用它，selector 字符串形式用 `RefOrValue<string>`。若因打包/bundle 原因不可导入，按 `packages/integrations/src/useFocusTrap.ts:71-78` 的本地别名先例（`MaybeElement` / `MaybeElementRef`）并上报，禁止静默复制。
...
> - **#114 `useDrauu`（上游已读，worktree `D:\reaxuse-wt\usedrauu` + `.agents\prompt-114.md` 已就绪，待空槽）**：返回**对象**（方法袋，非 §2B 元组）`{ drauuInstance, load, dump, clear, cancel, undo, redo, canUndo, canRedo, brush, setBrush, onChanged, onCommitted, onStart, onEnd, onCanceled }`；`drauuInstance: Drauu | undefined`（挂载 effect 内 `new Drauu({...defaults, ...options, el: unrefElement(target)})`，卸载 destroy，元素身份变化重建）；`canUndo`/`canRedo` 为布尔 state（从 drauu 事件重读 `instance.canUndo`/`.canRedo`）；`brush: Brush` + `setBrush`（对象内 setter 先例 `useNProgress`），`setBrush` 同时写 state 与 `instance.brush`；**5 个 `on*` 成员是 registrar，必须 `ListenerOn<...>` 类型 + 返回 `{ off }`**（§2D，先例 `packages/core/src/useFileDialog.ts:62-63`），消费方用 `useListener`；元素目标 `ElementTarget`（§2C）；上游默认 options `color:'black', size:3, arrowEnd:false, cornerRadius:0, dasharray:undefined, fill:'transparent', mode:'draw'`；**上游无测试文件**（测试自撰）；占位符 `packages/integrations/src/index.ts:13`。

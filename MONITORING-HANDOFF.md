...
> **PR #510** `feat(rxjs): add toObserver (#42)`（分支 `feat/rxjs-toobserver`，提交 `fa04fbe`，5 文件）——https://github.com/hairyf/reaxuse/pull/510 。**adjustment 协议已履行**：PR body 用 `Refs #42`（不写 `Closes`，**issue 不关闭**），并在 issue #42 下发布 React 适配说明评论（上游签名 → React 签名、`ObserverTarget<T> = MutableRefObject<T> | RefObject<T> | ((value: T) => void)`、仅 `{ next }` 无 `error`/`complete`、ref-like 写 `.current` 需单次 `as MutableRefObject<T>` 断言）。编排者复核通过：`packages/rxjs/src/index.ts` 仅第 9 行占位符翻转（第 14 行留给 #262）、0 处 `@reaxuse/core` 导入、0 处 meta 改动、0 处 `<DemoContainer>`（rxjs 不在 docs glob 内，见 §4）。CI 5/5 **首轮零重跑**（autofix 58s / build 1m3s / lint 1m12s / test 22.x 1m22s / test lts/* 1m24s），未合并。

...
| `setProgress`  | `Dispatch<SetStateAction<number \| null \| undefined>>` | Setter for `progress` (upstream: writing `progress.value`), accepts a plain value or a functional updater (`prev => next`); a number result is pushed to `nprogress` once, through the progress effect. |

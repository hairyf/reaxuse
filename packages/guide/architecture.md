...
| `playgrounds/build.sh`                    | ✅ [`playgrounds/build.sh`](https://github.com/hairyf/reause/blob/main/playgrounds/build.sh) (pnpm)                                              |
...
1. **Large-scale AI mapping** of all `@vueuse/core` functions — complete: [`meta/functions.md`](https://github.com/hairyf/reause/blob/main/meta/functions.md) lists every mapped export (310 rows: 295 resolved to an upstream path, 3 resolved outside the pinned submodule, 12 reause-only exports such as `isRefLike` or `writeState`).

...
| `.github/workflows/export-size.yml`          | export-size CI report via `antfu/export-size-action` — re-enabled now that the packages ship dist-based `exports` (the action's JS-only resolver used to fail on the old `./src/index.ts` entry fields)                 | ✅      |

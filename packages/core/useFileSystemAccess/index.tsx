 ...
 * object mirroring the upstream `{ isSupported, data, file, fileName,
 * fileMIME, fileSize, fileLastModified, open, create, save, saveAs,
 * updateData }` members. The hook wraps the browser's
 * `showOpenFilePicker` / `showSaveFilePicker` and keeps the picked file's
 * content (`data`, as `Text` / `ArrayBuffer` / `Blob` per the `dataType`
 * option) plus its `File` snapshot fresh.
 *
 * React divergences:
 * - the Vue shallow refs become plain state / derived values, so read
 *   `data`, `file`, `fileName`, `fileMIME`, `fileSize` and
 *   `fileLastModified` directly (no `.value`);
 ...
 *
 * @example
 * const { isSupported, open, save, data, file } = useFileSystemAccess({
 *   dataType: 'Text',
